...
The KiCad schematic is in [`kicad/kicad.kicad_sch`](../kicad/kicad.kicad_sch). Open it in KiCad 10 to view the full schematic and generate gerbers for PCB fabrication. Have it fabricated. 

It is possible to do this on perfboard: it is a good idea to breadboard it first and plan the perfboard version, use both sides (TODO: documentation of perfboard options).
...
Consider socketing the MCU using mill-max or machined pin headers instead of soldering it directly. This lets you swap or replace the board without desoldering.

Refer to the images on [breadboarding](images/breadboard/)
and on [PCB assembly](images/pcb_assembly/)

After assembly, [flash the firmware](HOW_TO_FIRMWARE.md)
...
| Display | 0.42" OLED (SSD1306) | I2C, 72×40 px, addr `0x3C` |
